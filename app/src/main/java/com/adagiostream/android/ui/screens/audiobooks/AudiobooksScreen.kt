...
/**
 * Audiobooks entry screen (beads_adagio-59p.1.4).
 *
 * With more than one book library this is the library picker; with exactly
 * one it forwards straight to that library's book list (replacing itself on
 * the back stack so Back returns to the Library tab, not an empty picker).
 *
 * @param onOpenLibrary navigate to the book list; `replace = true` means this
...
fun AudiobooksScreen(
    ...
    // Single book library — skip the picker entirely.
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (val s = state) {
            ...
            LibrariesState.Empty -> {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Text(
                        text = "No book libraries on your Audiobookshelf server.",
