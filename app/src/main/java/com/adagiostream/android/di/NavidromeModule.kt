...
abstract class NavidromeModule {

    /**
     * Binds [AccountManager] as the [AccountRepository] implementation so
     * components that only need read-only account observation (e.g.
     * [NavidromeLibraryViewModel]) can depend on the interface and be unit-tested
     * without constructing [AccountManager].
     */
    @Binds
    @Singleton
    abstract fun bindAccountRepository(manager: AccountManager): AccountRepository
